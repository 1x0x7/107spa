...
// ========================
// [공통 유틸] 재료 소모
// 보유 완제품 → 보유 중간재료 → 어패류 순으로 차감
// 항상 0 이상 보장
// ========================
function consumeInOrder(need, owned, intermediate, raw) {
  let remain = Math.max(0, need)

  const usedOwned = Math.min(owned, remain)
  remain -= usedOwned

  const usedIntermediate = Math.min(intermediate, remain)
  remain -= usedIntermediate

  const usedRaw = Math.min(raw, remain)
  remain -= usedRaw
  ...
  return {
    ...
    usedIntermediate,
    usedRaw,
    unmet: remain  // 재료가 부족한 경우 0보다 클 수 있음
  }
}

// ========================
// [공통 유틸] 실제 제작 가능한 제품 상한 계산
// 각 제품에 필요한 재료 소비량 기준으로 상한 계산
// ========================
function calcProductCap(totalResources, recipe) {
  let cap = Infinity
  for (const [resource, amount] of Object.entries(recipe)) {
    if (amount > 0) {
      cap = Math.min(cap, Math.floor((totalResources[resource] || 0) / amount))
    }
  }
...
function calculate1Star(input, isAdvanced, reservedCoreED = 0) {
  const shellfish = {
    guard: input.guard || 0, wave: input.wave || 0, chaos: input.chaos || 0,
    life: input.life || 0, decay: input.decay || 0
  }
  const ownedEss = {
  ...
  const ownedCore = isAdvanced ? {
    WG: input.coreWG || 0, WP: input.coreWP || 0, OD: input.coreOD || 0,
    VD: input.coreVD || 0, ED: input.coreED || 0
  } : { WG: 0, WP: 0, OD: 0, VD: 0, ED: 0 }

  const essFromShellfish = {
    guard: floorToTwo(shellfish.guard), wave: floorToTwo(shellfish.wave),
    chaos: floorToTwo(shellfish.chaos), life: floorToTwo(shellfish.life),
    decay: floorToTwo(shellfish.decay)
  }
  const totalEss = {
  ...
  // 각 제품별 실제 상한 계산
  // 보유 핵은 에센스 소모를 줄여주므로 핵 총량을 에센스 보너스로 더해 상한을 넉넉하게 잡음
  // (상한이 약간 넓어도 루프 내부 조건문이 실제 가능 여부를 정확히 검증하므로 결과에 영향 없음)
  const totalOwnedCores = ownedCore.WG + ownedCore.WP + ownedCore.OD + ownedCore.VD + ownedCore.ED
  const essWithCoreBonus = {
    guard: totalEss.guard + totalOwnedCores,
    wave:  totalEss.wave  + totalOwnedCores,
    chaos: totalEss.chaos + totalOwnedCores,
    life:  totalEss.life  + totalOwnedCores,
    decay: totalEss.decay + totalOwnedCores
  }

  const maxA = calcProductCap(essWithCoreBonus, { guard: 1, wave: 1, chaos: 1, life: 2, decay: 1 })
  const maxK = calcProductCap(essWithCoreBonus, { wave: 1, chaos: 2, life: 2, decay: 1 })
  const maxL = calcProductCap(essWithCoreBonus, { decay: 1, guard: 2, wave: 2, chaos: 1 })
  ...
  for (let A = 0; A <= maxA; A++) {
    for (let K = 0; K <= maxK; K++) {
      for (let L = 0; L <= maxL; L++) {
        ...
        if (
          needEss.guard > totalEss.guard || needEss.wave  > totalEss.wave  ||
          needEss.chaos > totalEss.chaos || needEss.life  > totalEss.life  ||
          needEss.decay > totalEss.decay
        ) continue
  ...
  const coreNeedProduct = {
    WG: best.A + best.L, WP: best.K + best.L,
    OD: best.A + best.K, VD: best.A + best.K, ED: best.L
  }
  ...
  const essNeedProduct = {
    ...
    chaos: craftCountProduct.chaos * 2, life:  craftCountProduct.life  * 2,
    decay: craftCountProduct.decay * 2
  }
  const blockNeedProduct = {
    clay: craftCountProduct.guard * 2, sand: craftCountProduct.wave * 4,
    dirt: craftCountProduct.chaos * 8, gravel: craftCountProduct.life * 4,
    granite: craftCountProduct.decay * 2
  }
  const fishNeedProduct = {
    shrimp: coreToMakeProduct.WG, domi: coreToMakeProduct.WP,
    herring: coreToMakeProduct.OD, goldfish: coreToMakeProduct.VD, bass: coreToMakeProduct.ED
  }
  ...
  const essNeedDilution = {
    guard: craftCountDilution.guard * 2, wave: 0, chaos: 0,
    life: 0, decay: craftCountDilution.decay * 2
  }
  const blockNeedDilution = {
    clay: craftCountDilution.guard * 2, sand: 0, dirt: 0,
    gravel: 0, granite: craftCountDilution.decay * 2
  }
  const fishNeedDilution = { shrimp: 0, domi: 0, herring: 0, goldfish: 0, bass: coreToMakeDilution }

  // 총합
  const coreNeed = {
    WG: coreNeedProduct.WG, WP: coreNeedProduct.WP,
    OD: coreNeedProduct.OD, VD: coreNeedProduct.VD,
    ED: coreNeedProduct.ED + reservedCoreED
  }
  const coreToMake = {
  ...
  const essToMake = {
    guard: craftCount.guard * 2, wave:  craftCount.wave  * 2,
    chaos: craftCount.chaos * 2, life:  craftCount.life  * 2,
    decay: craftCount.decay * 2
  }
  const essNeedTotal = { ...essToMake }
  const blockNeed = {
    clay: craftCount.guard * 2, sand: craftCount.wave * 4,
    dirt: craftCount.chaos * 8, gravel: craftCount.life * 4, granite: craftCount.decay * 2
  }
  const fishNeed = {
    shrimp: coreToMake.WG, domi: coreToMake.WP,
    herring: coreToMake.OD, goldfish: coreToMake.VD, bass: coreToMake.ED
  }
  const totalCraftCount = {
    guard: Math.ceil(essNeedTotal.guard / 2), wave:  Math.ceil(essNeedTotal.wave  / 2),
    chaos: Math.ceil(essNeedTotal.chaos / 2), life:  Math.ceil(essNeedTotal.life  / 2),
    decay: Math.ceil(essNeedTotal.decay / 2)
  }
  const blockNeedTotal = {
    clay: totalCraftCount.guard * 2, sand: totalCraftCount.wave * 4,
    dirt: totalCraftCount.chaos * 8, gravel: totalCraftCount.life * 4, granite: totalCraftCount.decay * 2
  }
  const fishNeedTotal = {
    shrimp: coreNeed.WG, domi: coreNeed.WP,
    herring: coreNeed.OD, goldfish: coreNeed.VD, bass: coreNeed.ED
  }
...
function calculate2Star(input, isAdvanced, reservedCrystalDefense = 0) {
  const shellfish = {
    guard: input.guard2 || 0, wave: input.wave2 || 0, chaos: input.chaos2 || 0,
    life: input.life2 || 0, decay: input.decay2 || 0
  }
  const ownedEss = {
    guard: input.essGuard || 0, wave: input.essWave || 0, chaos: input.essChaos || 0,
    life: input.essLife || 0, decay: input.essDecay || 0
  ...
  // 각 제품별 실제 상한 계산
  // 보유 결정은 에센스 소모를 줄여주므로 결정 총량을 에센스 보너스로 더해 상한을 넉넉하게 잡음
  const totalOwnedCrystals = ownedCrystal.vital + ownedCrystal.erosion + ownedCrystal.defense + ownedCrystal.regen + ownedCrystal.poison
  const essWithCrystalBonus = {
    guard: totalEss.guard + totalOwnedCrystals,
    wave:  totalEss.wave  + totalOwnedCrystals,
    chaos: totalEss.chaos + totalOwnedCrystals,
    life:  totalEss.life  + totalOwnedCrystals,
    decay: totalEss.decay + totalOwnedCrystals
  }

  const maxCORE   = calcProductCap(essWithCrystalBonus, { guard: 1, life: 2, wave: 2, decay: 1 })
  const maxPOTION = calcProductCap(essWithCrystalBonus, { wave: 2, decay: 2, life: 1, chaos: 1 })
  const maxWING   = calcProductCap(essWithCrystalBonus, { chaos: 2, guard: 2, decay: 1, life: 1 })
  ...
  const totalCrystalToMakeProduct = Object.values(crystalToMakeProduct).reduce((a, b) => a + b, 0)
  const totalEssToMakeProduct = Object.values(essNeedProduct).reduce((a, b) => a + b, 0)
  const materialNeedProduct = {
    ...
    cherryLeaves: craftCountProduct.life  * 6,
    DarkOakLeaves: craftCountProduct.decay * 6,
  ...
  const essNeedDilution = {
    guard: craftCountDilution.guard * 2, wave: 0,
    chaos: craftCountDilution.chaos * 2, life: 0, decay: 0
  }
  ...
  const crystalNeed = {
    ...
    defense: crystalNeedProduct.defense + reservedCrystalDefense,
  ...
  const essToMake = {
    ...
    chaos: craftCountTotal.chaos * 2, life:  craftCountTotal.life  * 2,
    decay: craftCountTotal.decay * 2
  }
  const essNeedTotal = { ...essToMake }
  const totalCrystalToMake = Object.values(crystalToMake).reduce((a, b) => a + b, 0)
  const totalEssToMake = Object.values(essToMake).reduce((a, b) => a + b, 0)
  const materialNeed = {
    ...
    cherryLeaves:  craftCountTotal.life  * 6,
    DarkOakLeaves: craftCountTotal.decay * 6,
...
function calculate3Star(input, isAdvanced, reservedPotionCorrupt = 0) {
  ...
  const ownedPotion = isAdvanced ? {
    ...
    corrupt:  input.potionCorrupt  || 0, frenzy:  input.potionFrenzy  || 0,
  ...
  // 각 제품별 실제 상한 계산
  // 보유 영약은 엘릭서 소모를 줄여주므로 영약 총량을 엘릭서 보너스로 더해 상한을 넉넉하게 잡음
  const totalOwnedPotions = ownedPotion.immortal + ownedPotion.barrier + ownedPotion.corrupt + ownedPotion.frenzy + ownedPotion.venom
  const elixWithPotionBonus = {
    guard: totalElix.guard + totalOwnedPotions,
    wave:  totalElix.wave  + totalOwnedPotions,
    chaos: totalElix.chaos + totalOwnedPotions,
    life:  totalElix.life  + totalOwnedPotions,
    decay: totalElix.decay + totalOwnedPotions
  }

  const maxAQUA     = calcProductCap(elixWithPotionBonus, { guard: 2, life: 1, wave: 2, decay: 1 })
  const maxNAUTILUS = calcProductCap(elixWithPotionBonus, { guard: 2, wave: 1, life: 2, chaos: 1 })
  const maxSPINE    = calcProductCap(elixWithPotionBonus, { chaos: 2, decay: 2, wave: 1, life: 1 })
  ...
  for (let AQUA = 0; AQUA <= maxAQUA; AQUA++) {
    for (let NAUTILUS = 0; NAUTILUS <= maxNAUTILUS; NAUTILUS++) {
      for (let SPINE = 0; SPINE <= maxSPINE; SPINE++) {
        ...
        if (
          needElix.guard > totalElix.guard || needElix.wave  > totalElix.wave  ||
          needElix.chaos > totalElix.chaos || needElix.life  > totalElix.life  ||
          needElix.decay > totalElix.decay
        ) continue
  ...
  const remainingOwnedElix = {
    chaos: Math.max(0, ownedElix.chaos - elixToMakeProduct.chaos),
    decay: Math.max(0, ownedElix.decay - elixToMakeProduct.decay)
  }
  ...
  const elixNeedDilution = {
    guard: 0, wave: 0,
    chaos: elixToMakeDilutionRaw.chaos,
    life: 0,
    decay: elixToMakeDilutionRaw.decay
  }
  ...
  const materialNeedDilution = {
    ...
    netherrack:  0,
    magmaBlock:  0,
    soulSoil:    elixToMakeDilutionRaw.chaos * 4,
    crimsonStem: 0,
    warpedStem:  elixToMakeDilutionRaw.decay * 4
  }
  const deadCoralNeedDilution = {
    deadTubeCoral:   0,
    deadBrainCoral:  0,
    deadBubbleCoral: potionToMakeDilution * 2,
    deadFireCoral:   0,
    deadHornCoral:   0
  }
  ...
  const potionNeed = {
    ...
    corrupt:  potionNeedProduct.corrupt + reservedPotionCorrupt,
...
function calculateAll(input, advanced1, advanced2, advanced3) {
  ...
  for (let d = 0; d <= maxDilution; d++) {
    ...
    const r1 = calculate1Star(
      { ...input.star1, ...(hasAdvanced1 ? advanced1 : {}) },
    ...
    const r3 = calculate3Star(
      { ...input.star3, ...(hasAdvanced3 ? advanced3 : {}) },
      !!hasAdvanced3, reservedPotionCorrupt
    )

    // [수정] null이어도 skip하지 않고 0으로 처리
    // → 희석액만 만드는 경우도 비교 대상에 포함
    const dilutionGold = d * GOLD_PRICES['0star'].DILUTION
    const star1Gold = r1 ? Math.max(0, r1.best.gold) : 0
    const star2Gold = r2 ? Math.max(0, r2.best.gold) : 0
    const star3Gold = r3 ? Math.max(0, r3.best.gold) : 0
